...
- [ ] In `dispute-pipeline.ts`, add `CASE_ACTION` and `DIALOGUE` to `ALLOWED_ARTIFACT_TYPES`
- [ ] In `runDisputePipeline`, after fetching case details, run in parallel:
...
- [ ] Build the Planner text payload to include three enrichment sections:
  ```
  ## Case actions
  ...
  ```
- [ ] For dialogue messages: pass **customer messages only** to the Planner — filter to messages where sender is the customer (not agent, not system/ANNABOT). The Planner doesn't need to see agent instructions or ANNABOT notes.
- [ ] Save `case_actions` as a separate jsonb column in `dispute_pipeline_runs` for audit trail — add to DB migration `init-db/003-...sql` (or create `004` if `003` already exists)
