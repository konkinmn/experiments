...
```typescript
{
  risk_level: "green" | "amber" | "red"   // from dispute profile
  what_customer_claims: string             // from dispute form
  key_signals: string[]                    // decisive signals from profile
  what_planner_considered: string          // condensed from thought
  what_agent_needs_to_decide: string       // specific open question
  uncertainty_factors: string[]            // from Planner output
  recommended_next_step: string            // not binding — advisory only
}
```
...
4. Pipeline re-runs when customer submits evidence (evidence artifacts now present)
...
customer_rationale: string  // short, controlled, template-constrained
                            ...
                            // generated by Planner but constrained by template
```
...
```typescript
{
  ...
  args: {
    is_dispute: boolean           // now true for formal disputes
    is_fraud: boolean
    credit_mode: CreditMode       // all modes now available
...
**New eligibility criteria for formal dispute path:**

To be defined with Payments team before Phase 4 starts. At minimum:
- Transaction ≥ £25 (anna-disputes minimum)
...
**Permission note:** `IMMEDIATELY` credits above £500 require `APPROVE_BIG_IMMEDIATE_CREDIT_PERMISSION`. The Executor service account needs appropriate permissions scoped to the cohort it handles. Define with Payments team.
...
This turns the pipeline into a stateful workflow engine. It requires:
- Persistent case state between pipeline runs
- SLA timers
- Conflict detection with human agents
- Loop termination logic

None of that complexity belongs in Phase 1-4. It is a meaningfully different system built on top of the validated foundation.
...
Phase 4 does not require Phase 3 to complete first — they can run in parallel if resource allows. Phase 5 requires both.
